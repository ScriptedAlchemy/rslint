...
- CI summary rendering consistency (markdown + JSON + npm wrapper parity, including strict-wrapper stdout parity, strict exit behavior, component-prefixed health-reason stderr contracts, silent-wrapper output-noise guards, and success-mode stderr silence, plus optional diff metrics when diff artifact exists),
- optional diff artifact summary consistency between markdown and JSON outputs,
- parity doctor output consistency (plain + markdown + JSON forms + npm wrapper parity + silent-wrapper output-noise guards + success-mode stderr silence),
