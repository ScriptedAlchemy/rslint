...
- CI summary rendering consistency (markdown + JSON + strict exit behavior + health-reason stderr contracts, including optional diff metrics when diff artifact exists),
...
- parity doctor strict-mode exit behavior (`--fail-on-critical` / `--fail-on-yellow`) with health-reason stderr contracts,
