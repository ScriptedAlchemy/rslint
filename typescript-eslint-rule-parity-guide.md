...
- CI summary rendering consistency (markdown + JSON + npm wrapper parity, including direct+strict-wrapper exact non-empty-line stdout parity against direct strict baselines, strict exit behavior, component-prefixed health-reason stderr contracts with line-for-line strict stderr parity, silent-wrapper output-noise guards, and success-mode stderr silence across direct+wrapper commands, plus optional diff metrics when diff artifact exists),
- optional diff artifact summary consistency between markdown and JSON outputs,
- parity doctor output consistency (plain + markdown + JSON forms + npm wrapper parity, including direct+strict-wrapper exact non-empty-line stdout parity against direct strict baselines + line-for-line strict stderr parity + silent-wrapper output-noise guards + success-mode stderr silence across direct+wrapper commands),
