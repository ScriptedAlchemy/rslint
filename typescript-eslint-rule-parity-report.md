...
6. Run consistency check to ensure tracker/summary/worklist/metadata/manifest/index/issue-plan/issue-body artifacts, CI summary output (markdown/JSON + strict exit behavior, including optional diff metrics when diff artifact exists), optional diff artifact consistency (markdown/JSON summaries), doctor output (plain/markdown/JSON + strict exit behavior), unified gate script exit/argument behavior (including missing threshold values), CI workflow trigger-path and diff-generation/diff-upload/artifact-bundle wiring, parity command inventory, command-reference backing-script mapping, and status/badge schema+strict-exit+arithmetic invariants remain synchronized.
