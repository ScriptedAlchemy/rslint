...
6. Run consistency check to ensure tracker/summary/worklist/metadata/manifest/index/issue-plan/issue-body artifacts, CI summary output (markdown/JSON + npm wrapper parity, including strict-wrapper stdout parity, strict exit behavior, component-prefixed health-reason stderr contracts, and silent-wrapper output-noise guards, plus optional diff metrics when diff artifact exists), optional diff artifact consistency (markdown/JSON summaries), doctor output (plain/markdown/JSON + npm wrapper parity, including strict-wrapper stdout parity + strict exit behavior + component-prefixed health-reason stderr contracts + silent-wrapper output-noise guards), unified gate script exit/argument/help behavior (including missing/invalid/duplicate threshold handling, duplicate `--skip-checks` handling, default-red behavior, `--threshold=<value>` parsing, reordered flag parsing, short-help support, usage-on-error output with threshold-form + skip-checks tokens, and quick-gate npm wrapper exit+skip-checks+threshold-marker+health-reason/success-marker contracts + prefixed-output parity with direct skip-check runs + silent-wrapper output-noise guards, all of which must not run strict clean checks), CI workflow trigger-path and diff-generation/diff-upload/artifact-bundle wiring, parity command inventory, command-reference backing-script mapping, and status/badge schema+strict-exit+status-write-stability+non-strict/strict/direct/wrapper write-line+arithmetic invariants (including npm wrapper strict status commands) remain synchronized.
