...
6. Run consistency check to ensure tracker/summary/worklist/metadata/manifest/index/issue-plan/issue-body artifacts, CI summary output (including optional diff metrics when diff artifact exists), doctor output (plain/markdown + strict exit behavior), CI workflow artifact-bundle wiring, parity command inventory, and status/badge schema+arithmetic invariants remain synchronized.
